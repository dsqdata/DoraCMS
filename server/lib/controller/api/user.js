...
exports.listUserConfig = async (req, res, next) => {
    try {
        let type = req.body.type;
        let openid = req.body.openid;
        let userLocal = await userConfigService.find(req.body, {query: {type: type, openid: openid}});
        renderSuccess(req, res, {data: userLocal});
    } catch (err) {
        renderFail(req, res, {
            message: err
        });
...
exports.addUserConfig = async (req, res, next) => {
    try {
        let fields = req.body;
        await userConfigService.create(fields);
...
exports.deleteUserConfig = async (req, res, next) => {
    try {
        let id = req.body.id;
        let currentUser = await userConfigService.removes(res, id);
        renderSuccess(req, res);
    } catch (err) {
        renderFail(req, res, {
            message: err
...
exports.listUserShareConfig = async (req, res, next) => {
    try {
        let resouce = await userShareConfigService.find({
            isPaging: '0'
        }, {
            query: {openid: req.body.openid},
            files: "openid"
        });
        renderSuccess(req, res, {data: resouce});
    } catch (err) {
        renderFail(req, res, {
            message: err
        });
...
exports.listUserShare = async (req, res, next) => {
    try {
        var title = req.body.content;
        const reg = new RegExp(title, 'i') //不区分大小写
        let userLocal = await userShareService.find(req.body, {query: {content: {$regex: reg}}});
        renderSuccess(req, res, {data: userLocal});
    } catch (err) {
        renderFail(req, res, {
            message: err
...
exports.addUserShare = async (req, res, next) => {
    try {
        let fields = req.body;
        let currentUser = await userShareService.create(fields);
        renderSuccess(req, res);
    } catch (err) {
        renderFail(req, res, {
            message: err
...
exports.deleteUserShare = async (req, res, next) => {
    try {
        let id = req.body.id;
        let currentUser = await userShareService.removes(res, id);
        renderSuccess(req, res);
    } catch (err) {
        renderFail(req, res, {
            message: err
...
exports.getCode = async (req, res, next) => {
    try {
        ...
        let appSecret = '36279f26f2f489688cba86f04a614327';
        let appId = 'wx0b59d637b6416073'
        ...
        let body = syncrequest('GET', url, {});
        let obj = JSON.parse(body.getBody('utf8'))
        renderSuccess(req, res, {data: obj});
    } catch (err) {
        renderFail(req, res, {
            message: err
        });
...
exports.getWxUserInfoById = async (req, res, next) => {
    try {
        let targetId = req.body.id;
        let doc = await userService.findByUsername({_id: targetId})
        renderSuccess(req, res, {
            data: doc.wxUserInfo
        });
...
exports.getUserLocal = async (req, res, next) => {
    try {
        let userLocal = await userService.find(req.body, {
            files: 'wxUserInfo.location wxUserInfo.nickName wxUserInfo.avatarUrl',
            query: {
                'wxUserInfo.location': {$ne: null}
            }
        });
...
exports.regActionWx = async (req, res, next) => {
    try {
        let fields = req.body || {};
        ...
        let doc = await userService.findByUsername({name: fields.openid})
        if (!!!doc) {
            const userObj = {
                name: fields.openid,
                userName: fields.nickName,
                logo: fields.avatarUrl,
                group: '0',
                creativeRight: false,
                password: service.encrypt(fields.nickName, settings.encrypt_key),
                loginActive: false,
                enable: true,
                wxUserInfo: fields
            }
            ...
            await userService.update(res, doc._id, {
                wxUserInfo: fields
            })
        }

        let user = await userService.item(res, {
            query: {name: fields.openid},
            files: getAuthUserFields('login')
        })
        let renderUser = JSON.parse(JSON.stringify(user));
